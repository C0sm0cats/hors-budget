...
(()=>{
  ...
  style.textContent=`
    .julien-board-overlay{position:fixed;z-index:12;transform:translate(-50%,-50%) rotate(-.35deg);pointer-events:none;overflow:hidden}
  ...
  board.innerHTML=`<svg viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g transform="translate(0 10)" font-family="Comic Sans MS, Bradley Hand, cursive" font-weight="700" fill="#29434a" stroke-linecap="round" stroke-linejoin="round">
  ...
  function loop(){
    ...
    if(s.level===1){
      ...
      const center=renderer.project(x,wy,-1.02),left=renderer.project(x-1.30,wy,-1.02),right=renderer.project(x+1.30,wy,-1.02),top=renderer.project(x,wy+.49,-1.02),bottom=renderer.project(x,wy-.49,-1.02);
      if(visible(center)&&left&&right&&top&&bottom){board.style.left=center.x+'px';board.style.top=center.y+'px';board.style.width=Math.max(205,Math.abs(right.x-left.x)*.96)+'px';board.style.height=Math.max(91,Math.abs(bottom.y-top.y)*.94)+'px';board.hidden=false;}else board.hidden=true;
      place(julienPlaque,renderer.project(x+1.92,wy+.02,-1.0));
      kevinPlaque.hidden=true;
    }else if(s.level===0){
      board.hidden=true;julienPlaque.hidden=true;
      place(kevinPlaque,renderer.project(3.62,4.72,-1.18));
