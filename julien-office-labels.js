...
(()=>{
  ...
  style.textContent=`
    ...
    .office-desk-plaque{position:fixed;z-index:18;transform:translate(-50%,-50%);padding:4px 8px 3px;border-radius:2px;background:#263943;color:#f3e3b8;border:2px solid #c7a86a;font:900 9px/1.05 system-ui;white-space:nowrap;pointer-events:none;box-shadow:0 2px 3px #0007,inset 0 0 0 1px #16242a}
    .office-desk-plaque small{display:block;margin-top:2px;color:#b9dce5;font-size:7px;letter-spacing:.035em;text-align:center}
  `;
  ...
  board.innerHTML=`<svg viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <g font-family="Comic Sans MS, Bradley Hand, cursive" font-weight="700" fill="#29434a" stroke-linecap="round" stroke-linejoin="round">
      ...
      <text x="22" y="29" font-size="10" fill="#8b3f43">12 %</text>
      <text x="53" y="43" font-size="10" fill="#8b3f43">9 %</text>
      <text x="82" y="56" font-size="10" fill="#8b3f43">6 %</text>
      <text x="111" y="68" font-size="10" fill="#8b3f43">3 %</text>
      <text x="140" y="91" font-size="11" fill="#327047">0 %</text>
      ...
      <text x="263" y="27" font-size="10" fill="#287aa6">marge ↑</text>
      <text x="247" y="64" font-size="10" fill="#9b467f">salaires →</text>
  ...
  const julienPlaque=document.createElement('div');
  julienPlaque.className='office-desk-plaque';
  julienPlaque.innerHTML='JULIEN<small>DTS · PAYS DE LA LOIRE</small>';
  julienPlaque.hidden=true;
  document.body.append(julienPlaque);

  const kevinPlaque=document.createElement('div');
  kevinPlaque.className='office-desk-plaque';
  kevinPlaque.innerHTML='KÉVIN<small>DIRECTEUR DE PROJETS</small>';
  kevinPlaque.hidden=true;
  document.body.append(kevinPlaque);
  ...
  function placePlaque(el,p){if(visible(p)){el.style.left=p.x+'px';el.style.top=p.y+'px';el.hidden=false;}else el.hidden=true;}
  ...
  function loop(){
    ...
    if(s.level===1){
      ...
      if(visible(center)&&left&&right&&top&&bottom){
        ...
        board.style.width=Math.max(215,Math.abs(right.x-left.x)*.98)+'px';
        board.style.height=Math.max(104,Math.abs(bottom.y-top.y)*.96)+'px';
        board.hidden=false;
      }else board.hidden=true;
      // Plaque fixée sur la façade du bureau, pas en bannière flottante sous le meuble.
      placePlaque(julienPlaque,renderer.project(x-.35,y+.34,.16));
      kevinPlaque.hidden=true;
    }else if(s.level===0){
      board.hidden=true;julienPlaque.hidden=true;
      // Le tableau blanc Kévin est à x=2 / y=4.65 ; la plaque est posée sur le bureau juste dessous.
      const x=2,y=surface(1,x);
      placePlaque(kevinPlaque,renderer.project(x-.2,y+.38,.12));
    }else{
      board.hidden=true;julienPlaque.hidden=true;kevinPlaque.hidden=true;
    }
