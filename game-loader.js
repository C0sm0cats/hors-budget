...
(()=>{
  ...
  const patchRequired=(from,to,label)=>{if(!src.includes(from))throw new Error(label+' introuvable');src=src.replace(from,to);};
  ...
  patchRequired("kevin:{name:'KÉKÉ',shirt:'#397cbd',hair:'#4b352c',skin:'#e6b88f',tie:'#fb536b'}","kevin:{name:'KÉKÉ',shirt:'#355d78',hair:'#4b352c',skin:'#e6b88f',tie:'#b94d61',internal:true,role:'projectDirector',pants:'#2e4051',shoes:'#19262f',blouse:'#f1eee6'}",'Tenue KÉKÉ');
  patchRequired("hugo:{name:'Hugo',shirt:'#d6aa58',hair:'#dfbe6c',skin:'#e9bd97',style:'quiff',demand:'Et mon variable ?'}","hugo:{name:'Hugo',shirt:'#c58f4f',hair:'#dfbe6c',skin:'#e9bd97',style:'quiff',category:'consultant',pants:'#3f5872',shoes:'#eeeae1',blouse:'#c58f4f',demand:'Et mon variable ?'}",'Tenue consultant Hugo');
  patchRequired("nora:{name:'Nora',shirt:'#609cbe',hair:'#211a18',skin:'#754b35',female:true,style:'bob',demand:'3 000 € par an !'}","nora:{name:'Nora',shirt:'#5c91aa',hair:'#211a18',skin:'#754b35',female:true,style:'bob',category:'consultant',pants:'#394f66',shoes:'#f0ede7',blouse:'#5c91aa',demand:'3 000 € par an !'}",'Tenue consultante Nora');
  patchRequired("basile:{name:'Basile',shirt:'#9b79a6',hair:'#1d1715',skin:'#8b5b3e',beard:true,demand:'Juste l’inflation !'}","basile:{name:'Basile',shirt:'#596879',hair:'#1d1715',skin:'#8b5b3e',beard:true,internal:true,category:'business',pants:'#303b46',shoes:'#1b232a',blouse:'#eee8df',tie:'#b9785e',demand:'Juste l’inflation !'}",'Tenue interne Basile');
  patchRequired("lea:{name:'Léa',shirt:'#77a389',hair:'#c96c36',skin:'#f0c49d',female:true,style:'bun',demand:'On en reparle quand ?'}","lea:{name:'Léa',shirt:'#765d79',hair:'#c96c36',skin:'#f0c49d',female:true,style:'bun',internal:true,category:'business',pants:'#4a3d4b',shoes:'#29252c',blouse:'#f2e9ed',demand:'On en reparle quand ?'}",'Tenue interne Léa');
  patchRequired("rodolphe:{name:'RORO',shirt:'#cb8864',hair:'#ba5d2f',skin:'#dfac84',beard:true,style:'curly',demand:'LES NAO COMMENCENT !'}","rodolphe:{name:'RORO',shirt:'#4a505a',hair:'#ba5d2f',skin:'#dfac84',beard:true,style:'curly',internal:true,role:'regionalDirector',pants:'#292e35',shoes:'#17191d',blouse:'#f0ebe1',tie:'#a45f53',demand:'LES NAO COMMENCENT !'}",'Tenue RORO');
  patchRequired("charline:{name:'CHACHA',shirt:'#e789b5',hair:'#674433',skin:'#efc39e',female:true,style:'long',princess:true}","charline:{name:'CHACHA',shirt:'#8d4f72',hair:'#674433',skin:'#efc39e',female:true,style:'long',internal:true,role:'businessManager',pants:'#e3b48f',shoes:'#2b1e2a',blouse:'#f8e9ef',skirt:'#59364f'}",'Tailleur CHACHA');

  patchRequired("scale=kind==='rodolphe'?1.27:kind==='kevin'?1.18:1.1;const part=","scale=kind==='rodolphe'?1.27:kind==='julien'?1.21:kind==='kevin'?1.18:kind==='charline'?1.12:1.1;const pants=p.pants||(p.internal?'#303d48':'#3d536b'),shoes=p.shoes||(p.internal?'#19242c':'#ede9df');const part=",'Proportions et palette vestimentaire');
  ...
  patchRequired("part(.115,.06,.055-.1*move,.2,.12,.28,'#1c3440');","part(.115,.06,.055-.1*move,.2,.12,.28,shoes);if(kind==='charline'){for(const side of [-1,1]){part(side*.115,.035,.14,.18,.07,.25,shoes);part(side*.17,.03,-.035,.045,.12,.045,shoes);}}else if(!p.internal){for(const side of [-1,1])part(side*.115,.025,.10,.22,.045,.30,'#f4f0e8');}",'Chaussures métier');
  ...
  patchRequired("part(0,.67,.15,.16,.35,.02,'#f6e8c9');","part(0,.67,.15,p.internal?.18:.12,p.internal?.36:.22,.025,p.blouse||p.shirt);if(p.internal){part(-.115,.72,.17,.075,.28,.022,p.shirt);part(.115,.72,.17,.075,.28,.022,p.shirt);}",'Chemise et revers de veste');
  patchRequired("part(kind==='kevin'?Math.sin(time*18)*Math.min(.14,Math.abs(Arcade.state.player.vx)*.025):0,.68,.167,.10,kind==='kevin'?.39:.22,.035,p.tie||'#ddba7c');","if(p.tie)part(kind==='kevin'?Math.sin(time*18)*Math.min(.14,Math.abs(Arcade.state.player.vx)*.025):0,.68,.167,.10,kind==='kevin'?.39:.22,.035,p.tie);",'Cravates réservées aux tenues concernées');
  patchRequired("if(p.style==='long'){part(0,.96,-.22,.45,.72,.1,p.hair);part(-.22,1.03,-.025,.085,.57,.27,p.hair);part(.22,1.03,-.025,.085,.57,.27,p.hair);}","if(p.style==='long'){part(0,.96,-.22,.45,.72,.1,p.hair);part(-.22,1.03,-.025,.085,.57,.27,p.hair);part(.22,1.03,-.025,.085,.57,.27,p.hair);}if(kind==='charline'){part(-.225,1.08,.19,.045,.075,.04,'#d5b26d');part(.225,1.08,.19,.045,.075,.04,'#d5b26d');part(0,.79,.175,.075,.09,.025,'#d5b26d');}",'Finition CHACHA');
  src=src.replaceAll('princess','chacha');

  const orderDecor="{x:-6.4,y:4.45,w:3.2,h:.68,title:'ORDRE DE MISSION'",swileDecor="{x:5.1,y:4.45,w:2.7,h:.68,title:'SWILE'";
  if(!src.includes(orderDecor)||!src.includes(swileDecor))throw new Error('Décors Ordre de mission / Swile introuvables');
  src=src.replace(orderDecor,"{x:-2.45,y:1.45,w:3.2,h:.68,title:'ORDRE DE MISSION'").replace(swileDecor,"{x:3.15,y:1.45,w:2.7,h:.68,title:'SWILE'");
  ...
  const decorativeOld="if(!s.boss.active)for(let i=0;i<3;i++)roll(moving,-9.35+i*.38,s.boss.y+.27,-.47,.21,.35,0);",movingOld="for(const b of s.barrels)roll(moving,b.x,b.y,.65,b.r,.48,b.spin);";
  if(!src.includes(decorativeOld)||!src.includes(movingOld))throw new Error('Appel tonneau introuvable');
  src=src.replace(decorativeOld,"if(!s.boss.active)for(let i=0;i<3;i++)esnObstacle(moving,-9.35+i*.55,s.boss.y+.38,-.47,s.level,false);");
  src=src.replace(movingOld,"for(const b of s.barrels)esnObstacle(moving,b.x,b.y,.65,s.level,true);");
 ...
 `;
  ...
  patchRequired("s.charlineIn-=dt;if(s.charlineIn<=0){s.charlineIn=rnd(12,17);","s.charlineIn-=dt;if(s.level===2&&s.charlineIn<=0){s.charlineIn=rnd(12,17);",'Aide café de CHACHA');
  patchRequired("notice('NIVEAU VALIDÉ · CHACHA LIBÉRÉE · '+LEVELS[s.level+1].name+' EN VUE !',3);","notice(s.level===0?'CHACHA EST AU POWER UP TOUR · DIRECTION EN VUE !':'JUJU EN ALIGNEMENT STRATÉGIQUE · ACCÈS AU ROOFTOP DÉBLOQUÉ !',3);",'Message de transition de niveau');
  patchRequired("function hud(){document.body.classList.toggle('top-floor',s.player.floor===4&&s.phase!=='title');","function hud(){document.body.dataset.level=String(s.level);document.body.classList.toggle('top-floor',s.player.floor===4&&s.phase!=='title');",'Synchronisation du niveau dans le DOM');
  patchRequired("s.phase==='transition'?'CHACHA LIBÉRÉE · ON CONTINUE !'","s.phase==='transition'?(s.level===0?'CHACHA EST AU POWER UP TOUR · DIRECTION EN VUE !':'ACCÈS AU ROOFTOP DÉBLOQUÉ !')",'Bandeau de transition');
  patchRequired("'DERNIER ÉTAGE · REJOINS CHACHA →'","(s.level===0?'DERNIER ÉTAGE · RETROUVE LA TRACE DE CHACHA →':s.level===1?'DERNIER ÉTAGE · PASSE JUJU · ACCÈS ROOFTOP →':'DERNIER ÉTAGE · REJOINS CHACHA →')",'Objectif du dernier étage');
