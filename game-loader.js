...
(()=>{
  ...
  // Le graphe marge/salaires n'appartient plus au tableau de Kévin : il est désormais chez Julien.
  src=src.replace(/      \/\/ Petit graphe au feutre bleu : marge en hausse, salaires à plat\.[\s\S]*?      c\.restore\(\);\n/, '');
  // Le tableau de Kévin est plus compact pour laisser une vraie zone de plaque de bureau à droite.
  src=src.replace("const x=2,y=4.65,w=3.5,h=1.7;","const x=2,y=4.65,w=2.8,h=1.7;");
 ...
 function julienOffice(mesh){const x=6.05,y=surface(1,x),z=-.72;mesh.box(x,y+.38,z,2.55,.12,1.0,'#6d5140');mesh.box(x-1.0,y-.02,z,.12,.8,.72,'#493a32');mesh.box(x+1.0,y-.02,z,.12,.8,.72,'#493a32');
   // Écran plus grand, relevé et avancé sur le bureau pour être immédiatement identifiable.
   mesh.box(x+.5,y+.73,z+.30,.98,.62,.10,'#253846');mesh.box(x+.5,y+.73,z+.36,.80,.44,.025,'#78b8c3');mesh.box(x+.5,y+.44,z+.28,.09,.28,.09,'#344b58');mesh.box(x+.5,y+.31,z+.25,.46,.055,.28,'#344b58');
   const wy=y+1.72,wz=-1.05;mesh.box(x,wy,wz,2.82,1.32,.07,'#f0f1eb');mesh.box(x,wy+.68,wz+.01,2.96,.055,.1,'#68757b');mesh.box(x,wy-.68,wz+.01,2.96,.055,.1,'#68757b');mesh.box(x-1.45,wy,wz+.01,.055,1.38,.1,'#68757b');mesh.box(x+1.45,wy,wz+.01,.055,1.38,.1,'#68757b');}
 `;
  ...
  const plantCalls=`const plantLayouts=[[[-8.15,0,0,.92],[7.55,2,1,.82],[-7.85,3,2,.9]],[[-7.05,0,2,.86],[3.8,1,0,.78],[-1.85,3,1,.8]],[[-8.3,0,1,.88],[8.2,1,2,.82],[-7.6,3,0,.9]]];for(const [px,pf,pk,ps] of plantLayouts[level])officePlant(world,px,surface(pf,px),-.72,pk,ps);if(level===1)julienOffice(world);`;
