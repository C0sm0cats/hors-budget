...
(()=>{
 ...
 function julienOffice(mesh){const x=6.05,y=surface(1,x),z=-.72;mesh.box(x,y+.38,z,2.55,.12,1.0,'#6d5140');mesh.box(x-1.0,y-.02,z,.12,.8,.72,'#493a32');mesh.box(x+1.0,y-.02,z,.12,.8,.72,'#493a32');mesh.box(x+.48,y+.58,z,.72,.46,.08,'#253846');mesh.box(x+.48,y+.58,z+.06,.58,.32,.025,'#6fa6b6');const wy=y+1.72,wz=-1.05;mesh.box(x,wy,wz,2.82,1.32,.07,'#f0f1eb');mesh.box(x,wy+.68,wz+.01,2.96,.055,.1,'#68757b');mesh.box(x,wy-.68,wz+.01,2.96,.055,.1,'#68757b');mesh.box(x-1.45,wy,wz+.01,.055,1.38,.1,'#68757b');mesh.box(x+1.45,wy,wz+.01,.055,1.38,.1,'#68757b');}
 `;
  ...
  // Les KPI de Julien se renvoient avec un CR, sur le même principe que les dossiers de Rodolphe.
  ...
  // Julien a le même type de nom fixe au-dessus de lui que les autres personnages principaux.
