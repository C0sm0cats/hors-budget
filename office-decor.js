...
(()=>{
  function desk(mesh,x,y,z,techServicesDirector){
    ...
    for(let i=0;i<3;i++){
      ...
      mesh.box(x-1.34,y+.69+i*.065,z+.14,.63,.018,.38,techServicesDirector?'#bbaa59':'#b95261');
    }
    if(!techServicesDirector){mesh.cylinder(x-.86,y+.76,z+.17,.13,.25,'#d9dac9',10);mesh.ring(x-.70,y+.77,z+.17,.09,.024,'#d9dac9');}
    const px=x+(techServicesDirector?-.70:1.10);
  ...
  function draw(mesh,sign,level,surface){
    ...
    desk(mesh,deskX,surface(1,deskX),-.65,techServicesDirector);
