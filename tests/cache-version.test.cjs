...
test('legacy runtime patch layers are gone and canonical assets are cache-busted',()=>{
  ...
  assert.ok(html.includes('game.js?v=50'));assert.ok(html.includes('polish.js?v=27'));assert.ok(html.includes('polish.css?v=19'));assert.ok(html.includes('banter-fair.js?v=23'));assert.equal(html.includes('roles-polish.js'),false);assert.equal(html.includes('<style>.rodolphe-glasses'),false);
