...
test('legacy runtime patch layers are gone and canonical assets are cache-busted',()=>{
  ...
  assert.ok(html.includes('game.js?v=54'));assert.ok(html.includes('polish.js?v=29'));assert.ok(html.includes('polish.css?v=22'));assert.ok(html.includes('banter-fair.js?v=26'));assert.equal(html.includes('roles-polish.js'),false);assert.equal(html.includes('<style>.regionalDirector-glasses'),false);
