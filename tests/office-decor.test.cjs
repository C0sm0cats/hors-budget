...
test('base office decor owns only KÉKÉ and JUJU boards',()=>{
  ...
  const ctx=vm.createContext({OfficePlaques:{draw(){}},OfficeWhiteboards:{draw(sign,key,x,y,z,h=1.8){const board={level:key==='juju'?1:0,floor:1,name:key==='juju'?'JUJU':'KÉKÉ',x,y,z,w:h*4/3,h,canvas:{}};ctx.OfficeBoards.push(board);return board;}},Path2D:class{constructor(path){assert.equal(typeof path,'string','missing handwritten character');assert.ok(path.length);}}});
